...
public class ComUtil {
	...
	public static void uniqe(ArrayList<Integer> words, ArrayList<Integer> tempUniqueWords,
			ArrayList<Integer> tempCounts) {
		for (int i = 0; i < words.size(); i++) {
			if (tempUniqueWords.contains(words.get(i))) {
				int index = tempUniqueWords.indexOf(words.get(i));
				tempCounts.set(index, tempCounts.get(index) + 1);
			} else {
				tempUniqueWords.add(words.get(i));
				tempCounts.add(1);
			}
		}
	}
